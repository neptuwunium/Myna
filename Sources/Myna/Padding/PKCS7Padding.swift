...
public struct PKCS7Padding: PaddingScheme {
	public func unpad(data: Data) throws -> Data {
		if let finalRemain = data.last {
			...
			guard finalRemain <= data.count else { throw MynaError.unexpectedPadding }

			let remain = data.reversed().count { value in value == finalRemain }

			guard remain == finalRemain else { throw MynaError.unexpectedPadding }
