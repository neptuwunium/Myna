...
public struct PCBCTransform: BlockCipherTransform {
	...
	public func encrypt(_ plainText: borrowing Data) throws -> Data {
		...
		try result.withUnsafeMutableBytes { resultPtr in
			...
			for block in blocks {
				...
				previousBlock.xor(inplace: workingBlock)
	...
	public func decrypt(_ cipherText: borrowing Data) throws -> Data {
		...
		var previousBlock = self.iv
		var tmpBlock = Data(count: blockSize)
		var workingBlock = Data(count: blockSize)
		...
		if let blocks = blocks {
			try result.withUnsafeMutableBytes { resultPtr in
				...
				let resultBase = resultBasePtr.assumingMemoryBound(to: UInt8.self)
				for block in blocks {
					tmpBlock.replaceSubrange(0 ..< blockSize, with: block)
					try algorithm.decrypt(&tmpBlock)

					workingBlock.replaceSubrange(0 ..< blockSize, with: tmpBlock)
					workingBlock.xor(inplace: previousBlock)

					previousBlock.replaceSubrange(0 ..< blockSize, with: workingBlock)
					previousBlock.xor(inplace: Data(block))
		...
		tmpBlock.replaceSubrange(0 ..< blockSize, with: finalBlock)
		try algorithm.decrypt(&tmpBlock)
		tmpBlock.xor(inplace: previousBlock)
		result.append(try padding.unpad(data: tmpBlock))
